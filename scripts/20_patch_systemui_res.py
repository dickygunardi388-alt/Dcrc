...
def set_integer(file_rel, name, value):
    f = find_res_file(file_rel)
    if not f:
        warn(f"{file_rel} tidak ditemukan — lewati set <integer name=\"{name}\">")
        return
    xml = read(f)
    # toleran terhadap atribut tambahan (mis. translatable="false") dan spasi
    # ekstra sebelum '>', dan hapus duplikat kalau ternyata ada lebih dari satu
    pattern = rf'<integer\s+name="{re.escape(name)}"[^>]*>.*?</integer>\s*\n?'
    replacement = f'    <integer name="{name}">{value}</integer>\n'
    matches = list(re.finditer(pattern, xml, re.DOTALL))
    if matches:
        # pertahankan hanya kemunculan pertama (dengan nilai baru), buang sisanya
...
def set_dimen(file_rel, name, value):
    f = find_res_file(file_rel)
    if not f:
        warn(f"{file_rel} tidak ditemukan — lewati set <dimen name=\"{name}\">")
        return
    xml = read(f)
    pattern = rf'<dimen\s+name="{re.escape(name)}"[^>]*>.*?</dimen>\s*\n?'
