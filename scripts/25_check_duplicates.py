...
FIX: sebelumnya hanya ngenalin format `<dimen name="X">`, jadi duplikat yang
kebentuk dari campuran `<dimen name="X">` + `<item type="dimen" name="X">`
(kasus real yang bikin build gagal) gak kedeteksi checker ini. Sekarang kedua
format dihitung sebagai resource yang sama.
"""
import re
import sys
...
DIRECT_RE = re.compile(r'<(dimen|integer|bool|string|color|plurals|drawable|id)\s+name="([^"]+)"')
# <item type="dimen" name="x" ...> ATAU <item name="x" type="dimen" ...> (urutan atribut bebas)
ITEM_RE = re.compile(
    r'<item\b(?=[^>]*\btype="(dimen|integer|bool|string|color|plurals|drawable|id)")'
    r'(?=[^>]*\bname="([^"]+)")'
)
